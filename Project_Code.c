...
int RIGHT_IR = A0; // RIGHT IR sensor connected to Analog pin A0
int LEFT_IR = A1; // LEFT IR sensor connected to Analog pin A1
...
if (analogRead(RIGHT_IR)<=35 && analogRead(LEFT_IR)<=35) {
digitalWrite(MOTOR_A1, LOW);
digitalWrite(MOTOR_A2, HIGH);
digitalWrite(MOTOR_B1, HIGH);
digitalWrite(MOTOR_B2, LOW);
}
else if (analogRead(RIGHT_IR)<=35 && !analogRead(LEFT_IR)<=35)
{
